<template>
  <v-container class="mt-3" fluid>
    <v-row>
      ...
      <v-col cols="12" sm="4" md="3">
        <v-select :items="customer" label="Customer" outlined dense hide-details></v-select>
      </v-col>
        ...
        <v-simple-table fixed-header height="205" class="mt-3" style="border: 1px solid grey">
          <template v-slot:default>
            ...
            <tr>
              ...
              <th id="rate" class="primary white--text text-center" style="width: 200px">Selling Rate (Rs.)</th>
            ...
            <tr v-for="(row, index) in rows" :key="index">
              <td>
                <v-select v-model="row.item" outlined hide-details dense :items="items" @change="isItemAlreadySelected(row,index)" />
              </td>
              <td>
      ...
      <v-col cols='12'>
        <v-card elevation='0' class="px-0">
          <v-card-actions class="px-0">
            <v-spacer></v-spacer>
            <v-btn color="primary" to="/purchase/return">
...
export default {
  ...
  data: () => ({
    customer: ['Medilanee Pvt. Ltd', 'Natural Pvt. Ltd', 'Clark Pvt.Ltd', 'Himalayan Nepal pvt. Ltd'],
    billType: ['VAT', 'PAN'],
